...
function buildProfile(pluginOutput) {
    ...
    function folder(parent, coordinate, child, title, color) {
        return action(
            ...
            title || child.name,
            { ProfileUUID: child.id },
            color,
            title ? 'Color' : child.name
        );
    }

    function back(target, coordinate) {
        return action(target, coordinate, 'com.elgato.streamdeck.profile.backtoparent', 'Back', {}, undefined, 'Back');
    }
    ...
    [test, prod, frames, blank].forEach((child, column) => {
        ...
        if (child === frames) {
            action(home, column + ',0', 'com.elgato.streamdeck.page.next', 'Frames', {}, undefined, 'Frames');
        } else {
    ...
    layout.frames.forEach((frame, column) => {
        ...
        action(
            ...
            'blank',
            'Blank\nSecondary',
            { frameIds: frame.id, feedType: 'secondary' },
            frame.color
        );

...
function send(event, context, payload) {
    ...
    deck.send(JSON.stringify({ event, context, ...(payload ? { payload } : {}) }));
}
...
function refresh(context) {
    ...
    send('sendToPropertyInspector', context, {
...
function configure(event) {
    ...
    if (previous && previous.connection && previous.connection.id === id) {
        ...
        previous.error = '';
        const client = previous.connection.client;
        if (event.event === 'didReceiveSettings' && client.ws.readyState === WebSocket.CLOSED) {
...
async function press(event) {
    ...
    try {
        if (!key.connection) {
            throw new Error(key.error || 'Configure connection first.');
        }
...
deck.on('message', async (raw) => {
    ...
    if (event.event === 'willAppear' || event.event === 'didReceiveSettings') {
        configure(event);
        ...
        refresh(event.context);
    } else if (event.event === 'propertyInspectorDidDisappear') {
        inspectors.delete(event.context);
...
deck.on('error', () => {
    /* Stream Deck owns plugin lifecycle */
});

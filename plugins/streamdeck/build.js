...
while (offset < icon.length) {
    ...
    if (length < 8 || offset + length > icon.length) {
        throw new Error('Invalid bundled ICNS icon.');
    }
    if (iconNames[type]) {
        const png = icon.subarray(offset + 8, offset + length);
        fs.writeFileSync(path.join(output, 'images', iconNames[type]), png);
        extracted.add(type);
    }
    offset += length;
}
...
if (extracted.size !== 2) {
    throw new Error('The app icon must contain 256px and 512px PNG images.');
}

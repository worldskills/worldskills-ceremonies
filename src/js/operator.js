(function () {
    ...
    app.controller(
        'OperatorCtrl',
        function ($scope, $timeout, $interval, RemoteTransport, SlideRowScope, SlideSnapshot) {
            var LAYOUT_KEY = 'ceremonator:operator:v1';
            var LOST_PENDING =
                'Connection lost before confirmation; command outcome unknown. Check the output after reconnecting.';
            ...
            $scope.saveLayout = function () {
                try {
                    localStorage.setItem(
                        LAYOUT_KEY,
                        JSON.stringify(angular.extend({}, $scope.layout, { frameId: $scope.frameId }))
                    );
                } catch (error) {
            ...
            function reconcileFrames(incoming) {
                ...
                angular.forEach(incoming, function (frame) {
                    frame.slides = SlideSnapshot.mergeSlides(
                        ($scope.screens[frame.id] || {}).slides,
                        frame.slides,
                        ($scope.feeds[0] || {}).id
                    );
                ...
                if (!$scope.frame && $scope.frames.length) {
                    ...
                    if (hadSelection) {
                        ...
                        reportDebug(
                            ...
                            'The Operator display for frame “' +
                                ($scope.frameId || 'unknown') +
            ...
            watchdog = $interval(function () {
                ...
                if (Date.now() - lastState > 15000) {
                    ...
                    clearPending(LOST_PENDING);
                    fail(
                        'No fresh control state for 15 seconds. Reconnecting; check that the control panel is running.'
                    );
        ...
        return function (scope, element) {
            ...
            function rowsChanged(changes) {
                return changes.some(function (change) {
                    ...
                    return Array.prototype.some.call(change.addedNodes, function (node) {
                        return (
                            node.nodeType === 1 &&
                            (node.matches('[data-operator-row], .list-group-item') ||
                                node.querySelector('[data-operator-row], .list-group-item'))
                        );
                    });
                });
            }
            ...
            unwatch = scope.$watchGroup(
                [
                    ...
                    'channel',
                    'frame.slideIndex',
                    'frame.previewSlideIndex',
                    'frame.slide.slideId',
                    'frame.previewSlide.slideId',
                    function () {
                ...
                function (next, previous) {
                    ...
                    if (next[0] !== previous[0] || next[1] !== previous[1] || next === previous) {
                        index =
                            scope.channel === 'preview' && frame.previewSlideIndex >= 0
                                ? frame.previewSlideIndex
                                : frame.slideIndex;
                    } else if (liveChanged) {
    ...
    app.directive('operatorPanel', function () {
        return function (scope, element) {
            ...
            function restore() {
                ...
                node.style.width = Number.isFinite(size.width)
                    ? Math.max(240, Math.min(3000, size.width)) + 'px'
                    : '100%';
                node.style.height = Number.isFinite(size.height)
                    ? Math.max(240, Math.min(2000, size.height)) + 'px'
                    : '360px';
            }
    ...
    app.directive('operatorMonitor', function ($timeout) {
        return function (scope, element) {
                ...
                if (event.data.type === 'operator-feed-error' || event.data.type === 'operator-feed-recovered') {
                    scope.$evalAsync(function () {
                        ...
                        if (event.data.type === 'operator-feed-recovered') {
                            ...
                            scope.feedErrors[key] =
                                'The “' + scope.feed.id + '” Operator feed reported: ' + event.data.error;
            ...
            scope.$watchGroup(['token', 'frameId', 'channel'], function () {
                ...
                $timeout.cancel(timer);
                iframe.src =
                    '/operator-feed.html?screen=' +
                    encodeURIComponent(scope.frameId) +
                    '&preview=true&feed=' +
                    scope.channel +
                    '&feedType=' +
                    scope.feed.id +
                ...
                timer = $timeout(function () {
                    var error =
                        'The “' + scope.feed.id + '” Operator monitor did not load. Reconnect or reload Operator.';

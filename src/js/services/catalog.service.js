        ...
        .factory(
            ...
            function ($filter, ResultFormat, Routing, SLIDE_KEYS, EXCEL_COLUMNS, ALBERT_VIDAL_AWARD_LABEL) {
                ...
                function sortedSponsors(skill) {
                    return (skill.sponsors || [])
                        .map(function (sponsor, index) {
                ...
                function build(input) {
                    ...
                    angular.forEach(skills, function (skill, i) {
                        var skillResults = Object.values(
                            results.filter(function (result) {
                                return (
                                    ResultFormat.normalizeSkillNum(result[EXCEL_COLUMNS.SKILL_NUMBER]) ===
                                    ResultFormat.normalizeSkillNum(skill.number)
                                );
                            })
                        );
                        ...
                        var skillMedalResults = groupByMember(
                            skillResults.filter(function (result) {
                                return (
                    ...
                    angular.forEach(members, function (member) {
                        var memberResult = (resultsBestOfNations || [])
                            .filter(function (result) {
                                return (
                                    result &&
                                    result[EXCEL_COLUMNS.MEMBER_NAME] &&
                                    result[EXCEL_COLUMNS.FIRST_NAME] &&
                                    result[EXCEL_COLUMNS.LAST_NAME] &&
                                    result[EXCEL_COLUMNS.MEMBER] == member.code
                                );
                            })
                            .reduce(
                                function (accumulator, result) {
                                    accumulator.competitors.push(
                                        ResultFormat.capitalize(result[EXCEL_COLUMNS.FIRST_NAME]) +
                                            ' ' +
                                            ResultFormat.capitalize(result[EXCEL_COLUMNS.LAST_NAME])
                                    );
                                    return accumulator;
                                },
                    ...
                    if (resultsBestOfNationMembers.length > 0) {
                        ...
                        for (var bon = 1; bon <= 99 && resultsBestOfNationMembers.length > 0; bon++) {
                            ...
                            var bestOfNationStates = bestOfNationSlice.map(function (r) {
                                return r.memberCode;
                            });
                            var slideBon = {
                    ...
                    var numericScores = results
                        .map(function (result) {
                            return parseFloat(result[EXCEL_COLUMNS.SCALE_SCORE]);
                        })
                        .filter(function (n) {
                            return !isNaN(n);
                        });

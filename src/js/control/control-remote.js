(function () {
    ...
    angular.module('ceremoniesApp').factory('RemotePart', function ($q, FrameService, FrameState) {
        return function ($scope) {
            ...
            var FRAME_ACTIONS = {
                resetPreview: function (action) {
                    $scope.resetPreview(action.frameId);
                },
                resetFrame: function (action) {
            ...
            var SLIDE_ACTIONS = {
                ...
                toggleState: function (action, slide) {
                    $scope.toggleState(action.frameId, slide, action.state);
                },
                resetStates: function (action, slide) {
            ...
            $scope.openOperator = function () {
                $q.when(window.ceremonator.remote.openOperator(FrameService.activeFrameId))
            ...
            $scope.saveRemoteConfig = function () {
                ...
                $q.when(
                    ...
                    .then(function (result) {
                        ...
                        $scope.addNotice(
                            'info',
                            'Remote settings saved. PIN: ' + result.config.pin + ', port: ' + result.config.port + '.',
                            'remote-config'
                        );
                    })
                    .catch(function (error) {
            ...
            function runStreamDeckCommand(command) {
                ...
                if (
                    ...
                    ids.some(function (id) {
                        return !Object.prototype.hasOwnProperty.call(FrameService.frames, id);
                    })
                ) {
                ...
                window.ceremonator.remote.onAction(function (action) {
                    ...
                    if (action && action.name === 'streamDeckCommand') {
                        $scope.$evalAsync(function () {
                            $q.when()
                                .then(function () {
                                    return runStreamDeckCommand(action.command);
                                })
                                .then(function (result) {
                                    window.ceremonator.remote.commandResult(action.requestId, result);
                                })
                                .catch(function (error) {
                                    window.ceremonator.remote.commandResult(action.requestId, {
                                        ok: false,

(function () {
    ...
    angular
        ...
        .factory(
            'ProjectPart',
            function ($q, Excel, FrameState, FrameService, ResultFormat, Routing, SessionSnapshot, SLIDE_KEYS) {
                return function ($scope) {
                                    ...
                                    if (skipped > 0) {
                                        $scope.addNotice(
                                            ...
                                            'Imported ' +
                                                ...
                                                ' result rows. ' +
                                                skipped +
                                                ' row(s) were skipped because a competitor name was missing.',
                                            'import'
                                        );
                                    } else {
                                        $scope.addNotice(
                                            'info',
                                            'Imported ' + total + ' result rows successfully.',
                                            'import'
                                        );
                                    }
                                });
                        ...
                        Excel.readRows(file)
                            .then(function (rows) {
                                $scope.$apply(function () {
                                    if (!rows || !rows.length) {
                                        $scope.addNotice(
                                            'error',
                                            'Import failed: no Best of Nation rows were found in that file. Please import a valid CIS spreadsheet (.xlsx).',
                                            'import-bon'
                                        );
                                        return;
                                    }
                                    ...
                                    $scope.addNotice(
                                        ...
                                        'Imported ' +
                                            rows.length +
                                            ' Best of Nation row(s), ' +
                                            $scope.bestOfNationGroupSize +
                                            ' per slide.',
                                        'import-bon'
                                    );
                                });
                            ...
                            .catch(function (error) {
                                $scope.$apply(function () {
                                    $scope.addNotice(
                                        'error',
                                        'Import failed: the Best of Nation file could not be read as a spreadsheet. Please import a valid CIS file (.xlsx). (' +
                                            (error && error.message ? error.message : 'unknown error') +
                                            ')',
                                        'import-bon'
                                    );
                    ...
                    $scope.importTranslations = function (file) {
                        ...
                        if (
                            !window.ceremonator ||
                            !window.ceremonator.project ||
                            !window.ceremonator.project.writeTranslations
                        ) {
                            $scope.addNotice(
                                'error',
                                'Import failed: translations are unavailable outside the desktop app.',
                                'import-translations'
                            );
                            return;
                        }
                        ...
                        Excel.readRows(file)
                            .then(function (rows) {
                                $scope.$apply(function () {
                                    if (!rows || !rows.length || typeof rows[0]['Key'] === 'undefined') {
                                        $scope.addNotice(
                                            'error',
                                            'Import failed: spreadsheet must have a "Key" column plus one column per language code.',
                                            'import-translations'
                                        );
                                        return;
                                    }

                                    var langCodes = Object.keys(rows[0]).filter(function (h) {
                                        return h !== 'Key';
                                    });
                                    var languages = {};
                                    var keyCount = 0;
                                        ...
                                        $scope.$apply(function () {
                                            if (!result || !result.ok) {
                                                $scope.addNotice(
                                                    'error',
                                                    'Import failed: ' +
                                                        (result && result.error ? result.error : 'no active project.'),
                                                    'import-translations'
                                                );
                                                return;
                                            }

                                            $scope.setScriptTranslations(languages);

                                            angular.forEach(FrameService.frames, function (frame, id) {
                                                if (
                                                    window.ceremonator &&
                                                    window.ceremonator.app &&
                                                    window.ceremonator.app.reloadScreen
                                                ) {
                                                    window.ceremonator.app.reloadScreen(id);
                                                }
                                            });

                                            var declaredCodes = ($scope.languages || []).map(function (l) {
                                                return l.lang_code;
                                            });
                                            var undeclared = langCodes.filter(function (c) {
                                                return declaredCodes.indexOf(c) < 0;
                                            });
                                            ...
                                            if (undeclared.length) {
                                                $scope.addNotice(
                                                    'warning',
                                                    "Column(s) not listed in this project's languages: " +
                                                        undeclared.join(', ') +
                                                        ". Saved, but won't show on screen until added to project.json → languages.",
                                                    'import-translations'
                                                );
                                            }
                                        });
                            ...
                            .catch(function (error) {
                                $scope.$apply(function () {
                                    $scope.addNotice(
                                        'error',
                                        'Import failed: the file could not be read as a spreadsheet. Please import a valid Key/language-code file (.xlsx). (' +
                                            (error && error.message ? error.message : 'unknown error') +
                                            ')',
                                        'import-translations'
                                    );
                    ...
                    $scope.importOrdering = function (file) {
                        ...
                        Excel.readRows(file)
                            .then(function (rows) {
                                ...
                                if (
                                    typeof rows[0]['Skill Number'] === 'undefined' ||
                                    typeof rows[0]['Frame Name'] === 'undefined'
                                ) {
                                    alert(
                                        'Import failed: spreadsheet must have "Skill Number" and "Frame Name" columns.'
                                    );
                                    return;
                                }
                                ...
                                $scope.$apply(function () {
                                    ...
                                    rows.forEach(function (row, index) {
                                        ...
                                        if (
                                            skillNum === SLIDE_KEYS.ALBERT_VIDAL ||
                                            skillNum === SLIDE_KEYS.BEST_OF_NATION
                                        ) {
                                            return;
                                        }
                                    ...
                                    FrameService.setSkillOrder(
                                        entries.map(function (entry) {
                                            return entry.skillNumber;
                                        })
                                    );
                                    ...
                                    entries.forEach(function (entry) {
                                        ...
                                        var frameId = entry.frameName
                                            ? labelToId[entry.frameName.toLowerCase()] || null
                                            : null;
                                    ...
                                    $scope.addNotice(
                                        'info',
                                        'Imported ordering for ' + entries.length + ' skill(s).',
                                        'ordering-import'
                                    );
                                });
                            })
                            .catch(function (error) {
                                $scope.$apply(function () {
                                    $scope.addNotice(
                                        'error',
                                        'Ordering import failed: ' +
                                            (error && error.message ? error.message : 'invalid spreadsheet'),
                                        'ordering-import'
                                    );
                                });
                            });
                    ...
                    $scope.saveProject = function () {
                        ...
                        var doSave = function () {
                            FrameService.saveProject(
                                $scope.projectName || 'Ceremony Project',
                                $scope.displayMode || 'windows',
                                $scope.gridConfig,
                                $scope.languages,
                                $scope.bestOfNationGroupSize,
                                $scope.remoteConfig,
                    ...
                    $scope.reloadProject = function () {
                        $scope.projectMenuOpen = false;
                        if ($scope.projectReloading) return;
                        if ($scope.projectDirty && !confirm('Reload project settings and discard unsaved changes?'))
                            return;

                        var projectApi = window.ceremonator && window.ceremonator.project;
                        if (!projectApi || !projectApi.current || !projectApi.openPath) {
                            $scope.addNotice('error', 'Project reload is unavailable.', 'project-reload');
                            return;
                        }

                        $scope.projectReloading = true;
                        return $q
                            .when(projectApi.current())
                            .then(function (current) {
                                if (!current || !current.dir) throw new Error('No active project open.');
                                return projectApi.openPath({ dir: current.dir });
                            })
                            .then(function (result) {
                                if (!result || !result.ok) {
                                    throw new Error((result && result.error) || 'Project could not be reloaded.');
                                }
                                $scope.applyProjectConfig(result);
                                return $scope.loadCatalogs();
                            })
                            ...
                            .then(function () {
                                ...
                                $scope.addNotice('info', 'Project settings reloaded from disk.', 'project-reload');
                            })
                            .catch(function (error) {
                                $scope.addNotice(
                                    'error',

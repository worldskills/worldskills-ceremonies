(function () {
    ...
    angular
        ...
        .factory(
            'FramesPart',
            function (FrameService, FrameState, QueueScroll, SlideStep, FRAMES_WINDOW_STATUS, FEED, WORKSPACE_MODES) {
                return function ($scope) {
                    ...
                    $scope.feedWindowCount = function (frame, feedType, channel) {
                        ...
                        return feedType === FrameService.primaryFeedId() && frame && frame.windows
                            ? frame.windows[channel] || 0
                            : 0;
                    };

                    function openFrameWindowContainer(frameId, frame, isPreview, container, feedType) {
                        window.ceremonator.frames
                            .openWindow({
                                frameId: frameId,
                                size: frame.size,
                            ...
                            .catch(function (error) {
                                $scope.$apply(function () {
                                    ...
                                    $scope.addNotice(
                                        'error',
                                        'Could not open output: ' + (error.message || error),
                                        'open-output-' + frameId
                                    );
                                });
                            });
                    ...
                    $scope.openFrameWindow = function (frameId, isPreview, feedType) {
                        ...
                        if (window.ceremonator && window.ceremonator.frames) {
                            if (
                                $scope.gridConfig.splitContainers &&
                                !isPreview &&
                                (feedType || FrameService.primaryFeedId()) === FrameService.primaryFeedId()
                            ) {
                            ...
                            var url =
                                ...
                                (isPreview ? '&preview=true&feed=' + FEED.PREVIEW : '') +
                                '&label=' +
                                encodeURIComponent(frame.label || frameId) +
                                ($scope.testMode ? '\&testMode=1' : '');
                    ...
                    $scope.canOpenFramePreview = function (frameId, feedType) {
                        ...
                        var counts =
                            ...
                            frame.windows &&
                            frame.windows.feeds &&
                            frame.windows.feeds[feedType || FrameService.primaryFeedId()];
                        return (
                            !!frame &&
                            !!(counts ? counts.live : frame.windows && frame.windows.live) &&
                            !(counts ? counts.preview : frame.windows && frame.windows.preview)
                        );
                    };
                    ...
                    $scope.openFrameWindowPreview = function (frameId, feedType) {
                        ...
                        if (!(counts ? counts.live : frame.windows && frame.windows.live)) {
                            $scope.addNotice(
                                'warning',
                                'Open a Live window for "' + (frame.label || frameId) + '" before opening Preview.',
                                'preview-needs-live'
                            );
                            return;
                        }
                        if (counts ? counts.preview : frame.windows && frame.windows.preview) {
                            $scope.addNotice(
                                'warning',
                                'Preview is already open for "' +
                                    (frame.label || frameId) +
                                    '" — only one Preview window per frame is allowed.',
                    ...
                    $scope.previewAllFrames = function () {
                        ...
                        if (!eligible.length) {
                            $scope.addNotice(
                                'warning',
                                'No frames are eligible for Preview — open Live windows first (and close any Preview already open).',
                                'preview-all-none'
                            );
                            return;
                        }
                        ...
                        if (skipped.length) {
                            $scope.addNotice(
                                'warning',
                                'Skipped (no Live window open, or Preview already open): ' + skipped.join(', '),
                                'preview-all-skipped'
                            );
                        }
                    };
                    ...
                    $scope.getGridCols = function () {
                        ...
                        return $scope.gridConfig.splitContainers && $scope.isPrimaryGridFeed()
                            ? frameCols * 2
                            : frameCols;
                    };
                    ...
                    $scope.confirmOpenGridView = function () {
                        ...
                        var monitor =
                            $scope.gridConfig.monitors[$scope.gridConfig.feedType || FrameService.primaryFeedId()];
                        var monitorOverride =
                            monitor === null || monitor === undefined || monitor === '' ? null : parseInt(monitor, 10);
                            ...
                            .then(function (result) {
                                if (!result || result.ok === false)
                                    $scope.$applyAsync(function () {
                                        $scope.addNotice(
                                            'warning',
                                            (result && result.error) || 'Could not open Grid view.',
                                            'grid-open'
                                        );
                                    });
                            });

(function () {
    ...
    angular
        ...
        .factory('SessionPart', function ($q, FrameService, FrameState, SessionSnapshot, WORKSPACE_MODES) {
            return function ($scope) {
                ...
                $scope.clearAllData = function () {
                    if (
                        !confirm(
                            "Clear all session data? This discards the current run's progress (imported results, frame assignments, live/preview state) so the app restarts fresh. Your project files, templates and skill/member data are not affected."
                        )
                    ) {
                        return;
                    }
                    ...
                    return $q.when(SessionSnapshot.load()).then(function (saved) {
                        ...
                        $scope.addNotice(
                            ...
                            'Restored previous session — ' +
                                ...
                                ' result row(s), ' +
                                Object.keys(FrameService.frames).length +
                                ' frame(s).',
                            'dev-session'
                        );
                        return true;
                    ...
                    SessionSnapshot.registerCollector(function () {
                        return {
                            ...
                            ui: {
                                ...
                                selectedSkillNumber: $scope.skillsSelectedSkill
                                    ? $scope.skillsSelectedSkill.number
                                    : null,
                            },
                        };
                    });

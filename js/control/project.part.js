(function () {
    ...
    angular.module('ceremoniesApp').factory('ProjectPart', function (Excel, FrameState, FrameService) {
        return function (scope) {
            ...
            // Import skill/medal name translations: a Key column (the English string,
            // as it appears in the template) plus one column per language code.
            // Written straight to <project>/translations.json, independent of Project → Save.
            ...
            /**
             * Import skill ordering for correct display skills in order we want
             * @param file
             * Excel format
             *
            ...
            scope.saveProject = function () {
                scope.projectMenuOpen = false;
                var doSave = function () {
                    FrameService.saveProject(scope.projectName || 'Ceremony Project', scope.displayMode || 'windows', scope.gridConfig, scope.languages).then(function (result) {
                        if (result && result.ok) {
            ...
            scope.saveAsProject = function () {
                scope.projectMenuOpen = false;
                FrameService.saveAsProject(scope.projectName || 'Ceremony Project', scope.displayMode || 'windows', scope.gridConfig, scope.languages).then(function (result) {
            ...
            scope.exitToStartup = function () {
                var message = 'Return to the start screen? This closes ceremonator opened screens and control panel.';
